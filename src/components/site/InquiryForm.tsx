...
export function InquiryForm({
  ...
  const t = useCommon().inquiryForm;
  const locale = useLocale();

  const schema = z.object({
    name: z.string().trim().min(1, t.errorNameRequired).max(100),
    email: z.string().trim().email(t.errorEmailInvalid).max(255),
  ...
  if (sent) {
    return (
      <div className={`rounded-3xl p-8 text-center ${dark ? "bg-white/10 text-white" : "bg-card shadow-sm"}`}>
        ...
        <p className={`mt-2 text-sm ${dark ? "text-white/70" : "text-muted-foreground"}`}>{t.successBody}</p>
        <a href={SITE.whatsapp} target="_blank" rel="noopener noreferrer" className="btn-accent mt-5">
          <MessageCircle className="h-4 w-4" /> {t.continueWhatsapp}
        </a>
      </div>
    );
  }
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        const { error } = await supabase.from("trip_requests").insert({
          ...
          source: context ? `${context}${locale === "ja" ? "-ja" : ""}` : locale === "ja" ? "inquiry-form-ja" : "inquiry-form",
    ...
    >
      ...
      <div>
        <label htmlFor={`inq-name-${context ?? "gen"}`} className="sr-only">{t.nameSr}</label>
        <input
      ...
      <div>
        <label htmlFor={`inq-email-${context ?? "gen"}`} className="sr-only">{t.emailSr}</label>
        <input
      ...
      {!compact && (
        <div>
          <label htmlFor={`inq-msg-${context ?? "gen"}`} className="sr-only">{t.messageSr}</label>
          <textarea
      ...
      <p className={`text-center text-xs ${dark ? "text-white/60" : "text-muted-foreground"}`}>{t.disclaimer}</p>
    </form>
  );
}
